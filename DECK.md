...
- MUST bound the total uncompressed size, the number of entries, and the ratio of uncompressed to compressed size. A ratio limit of 100:1 is RECOMMENDED; card artwork ([§5.7.4](#574-the-extension-chain)) does not approach it. This specification fixes no absolute limit.
...
The unpacked `mimetype` file is ignored by discovery ([§5.7.1](#571-image-roots)), as any other non-asset file is.
...
Declare the term for how the artwork principally came to exist, or, where a later operation changed what the image depicts, the term for that operation. Routine preparation — deskewing, cropping, dust removal, color correction — does not change the term.
...
| `description` | String | No | none | Prose about the design, such as its provenance, for display alongside the back in a picker or an info panel. Not localized; written once in the deck's [`metadata_language`](#41-deck) ([§6.2](#62-language-resolution)). |
...
A card that declares `unnumbered = true` carries no number whatever its key shape implies, and an application MUST NOT present one for it. `number` and `unnumbered` answer the same question and a card MUST NOT declare both. **Ordering is unaffected**: a major arcanum with a two-digit key keeps the implicit `position` of [§4.3.2](#432-ordering), so a card can sit in the numbered sequence and print no number, as the Fool does in the Marseille pattern.
...
An application MUST NOT resolve a card against a reference deck for a package that declares [`rel = "expands"`](#419-related-decks). Such a package is a set of cards added to another deck rather than a deck in its own right, so every canonical slot it does not carry is a slot it was never meant to carry, and filling them from a reference deck presents someone else's complete deck under the expansion's name.
...
2. Ranks the image roots of the requested kind by [§5.7.3](#573-size-selection-within-a-kind) and, best first, looks in `<root>/<subpath>` for that stem under the [extension chain](#574-the-extension-chain), taking the first file it can decode.
3. Failing that, and where a variant was requested, repeats step 2 for the card's default variant ([§5.7.5](#575-variants)).
4. Failing that, resolves the card against the [reference deck](#13-terminology) where the library has one, the card is not excluded, and the card has a canonical counterpart ([§5.7.6](#576-when-no-asset-is-found)). A reference deck is never consulted in the middle of size selection.
...
Two facets are defined, `name` and `alt_text`. Together with the reserved [`[metadata]`](#621-name-file-metadata) they are the only top-level tables a name file carries and a validator SHOULD report any other ([§9.4](#94-validation-rules)) rather than ignore it. An unrecognized facet is an error and not the silent ignoring of [§8](#8-extensibility), because it supplies no strings at all and a fully translated deck would present as an untranslated one.
...
| **E** | Every `[card_backs.designs]` table key is a well-formed [custom name](#32-custom-names), and every `image` path declared under it exists. A discovered stem is not covered by this rule; the warning below reports it ([§5.5](#55-card-back-images)). |
...
| **W** | A card back design supplied in no [baseline format](#574-the-extension-chain). An application that cannot decode it substitutes its own back and the design is never seen ([§5.5](#55-card-back-images)). |
...
| **E** | No card declares both `name` and `unnamed = true` ([§6.3](#63-display-name-resolution)). |
...
| **W** | A card's `inscription` equal to its resolved `number` or to its resolved name. The definition excludes both, so the string will be rendered twice ([§4.3.3](#433-inscriptions)). |
...
| **W** | An [extended major arcanum](#13-terminology) no name file names. Nothing else can name it ([§6.3](#63-display-name-resolution)), so it is shown to the user as a bare number. |
...
| **W** | A `redistribution` or `derivation` narrower than `full` on a deck whose `license` is a public license granting redistribution or derivation outright. The license governs and the field does not take it back ([§7.5](#75-redistribution-and-derivation)), so the field misleads a reader without binding anyone. A validator checks this only for licenses it recognizes. |
| **W** | A [surrogate deck](#59-surrogate-decks) declaring `redistribution = "full"`. The field governs passing on the artwork and the package carries none ([§5.9](#59-surrogate-decks)). A `license` on such a deck is not reported; it covers the surrogates. |
...
| **W** | An `isbn` that is not ten or thirteen characters once hyphens and spaces are removed, or whose check digit does not verify. Box copy is transcribed by hand and this catches the transcription error ([§4.1.5](#415-product-identifiers)). |
...
| **W** | A `product_ids` scheme outside the registry of [§4.1.5](#415-product-identifiers) that is not prefixed (as above). |
...
| **E** | Within an `oars-1.1` subtable, every descriptor key is one of the twenty-three OARS 1.1 attribute ids written with underscores and every value is one of `none`, `mild`, `moderate` or `intense` ([§4.1.6](#416-content-rating)). |
| **E** | Every rating system named on an [artwork](#13-terminology) is a system `[deck.content_rating]` also declares, and no descriptor declared on an artwork exceeds the deck-level value declared for the same system and descriptor under the ordering `none` < `mild` < `moderate` < `intense` ([§4.1.6](#416-content-rating)). A descriptor the deck-level subtable omits is `none`, and so admits no value above it on any artwork, except under `artwork_complete = true`, where it constrains nothing. |
| **W** | A `content_rating` system outside the registry of [§4.1.6](#416-content-rating) that is not prefixed (as above). |
| **E** | Where any [artwork](#13-terminology) declares a descriptor for a system, that system's subtable in `[deck.content_rating]` declares `artwork_complete` ([§4.1.6](#416-content-rating)). |
| **W** | Under `artwork_complete = true`, a declared deck-level descriptor whose value exceeds every value the deck's artwork carries for it. One of the two is unfinished ([§4.1.6](#416-content-rating)). A deck-level descriptor that merely restates what the artwork already carries is not reported. |
...
| **W** | A value under an `iptc-dst` key that is not a term name of the IPTC Digital Source Type vocabulary ([§4.1.8](#418-artwork-origin)). The vocabulary is not this specification's to close, so an unrecognized term is ignored rather than rejected and a misspelled term passes quietly. |
| **W** | An `origin` system outside the registry of [§4.1.8](#418-artwork-origin) that is not prefixed (as above). |
...
- Replaces `created_date` and `updated_date` with [`published_date`](#417-published-date).
